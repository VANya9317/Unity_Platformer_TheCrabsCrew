    ...
    public class Hero : Creature, ICanAddInInventory
    {
        ...
        private void OnCollisionEnter2D(Collision2D other)
        {
            if (other.gameObject.IsInLayer(_groundLayer))
            {
                ...
                if (contact.relativeVelocity.y >= _downVelocity)
                {
        ...
        public void Batyt(float batytSpeed)
        {
            _isJumping = false;
            Rigidbody.velocity = new Vector2(Rigidbody.velocity.x, batytSpeed);
        }
        
        public void UseBlink()
        {
            var blink  = FindObjectOfType<BlinkProjectile>().transform;
            Blink(blink);
        }
        
        private void Blink(Transform blinkEye)
        {
            transform.position = Vector3.Lerp(transform.position, blinkEye.position, 1f);
        }
    }
}
